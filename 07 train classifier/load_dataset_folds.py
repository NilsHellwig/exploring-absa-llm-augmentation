...
def load_dataset_folds(LLM_NAME, N_REAL, N_SYNTH, LLM_SAMPLING):
    ...
    for i in range(1, constants.N_FOLDS+1):
        with open(f'../07 train classifier/real/split_{constants.SPLIT_LOOP[i]}.json', 'r') as json_datei:
    ...
    if LLM_SAMPLING == "random" or N_SYNTH == 0:
        ...
        for idx_real in range(constants.N_FOLDS):
            train_dataset.append([])
            with open(f'../07 train classifier/real/split_{idx_real}.json', 'r') as json_datei:
                real_split = json.load(json_datei)
    ...
    if N_SYNTH > 0:
        for split_id in range(constants.N_FOLDS):
            with open(f'../07 train classifier/synth/{LLM_NAME}/{LLM_SAMPLING}/split_{constants.SPLIT_LOOP[split_id]}.json', 'r') as json_datei:
