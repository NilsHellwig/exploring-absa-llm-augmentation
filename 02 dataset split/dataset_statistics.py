...
def df_statistics(dataset_json, synthetic=False):
    """
    Computes various corpus-level statistics for the given dataset.

    Args:
        dataset_json (list): List of sample dictionaries.
        synthetic (bool): Flag indicating if statistics for synthetic data should be included.
    ...
    if synthetic:
        # Additional statistics for synthetic datasets
        statistics = get_statistics_for_synthetic_data(
            statistics, dataset_json)

    # Compute per-category statistics for explicit, implicit, and total aspects
    for ac in ASPECT_CATEGORIES:
        ...
        statistics[f"number_of_implicit_aspects_{ac}"] = len(
            [tag for entry in dataset_json for tag in entry['tags'] if tag['type'] == 'label-implicit' and tag["label"] == ac])

        statistics[f"number_of_explicit_aspects_{ac}"] = len(
            [tag for entry in dataset_json for tag in entry['tags'] if tag['type'] == 'label-explicit' and tag["label"] == ac])

        statistics[f"number_of_aspects_{ac}"] = len(
            [tag for entry in dataset_json for tag in entry['tags'] if tag["label"] == ac])

    # Global terminology and aspect density statistics
    ...
    statistics["number_of_implicit_aspects"] = len(
        [tag for entry in dataset_json for tag in entry['tags'] if tag['type'] == 'label-implicit'])
    ...
    for ac in ASPECT_CATEGORIES:
        for polarity in POLARITIES:
            for mention_type in ["label-explicit", "label-implicit"]:
                statistics[f"{ac}-{polarity}-{mention_type}-frequency"] = len(
                    [tag for example in dataset_json for tag in example["tags"] if tag["polarity"] == polarity and tag["label"] == ac and tag["type"] == mention_type])
...
def get_statistics_for_synthetic_data(statistics, dataset_json):
    ...
    for retry_reason in ["llm_invalid_xml_schema", "llm_invalid_xml_tags", "llm_aspect_polarity_in_text_but_not_in_label", "llm_no_token_in_sentence", "llm_more_than_one_sentences", "llm_empty_aspect_term", "llm_invalid_single_word_aspect_term_pos_tag"]:
