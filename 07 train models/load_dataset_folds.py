...
def load_dataset_folds(LLM_NAME, N_REAL, N_SYNTH, LLM_SAMPLING, random):
    ...
    # 2. Load Test Data
    test_dataset = []
    for i in range(0, constants.N_FOLDS):
    ...
    if LLM_SAMPLING == "random" or N_SYNTH == 0:
        ...
        for idx_real in range(constants.N_FOLDS):
            ...
            with open(f'../07 train models/real/split_{constants.SPLIT_LOOP[1:constants.N_FOLDS+1][idx_real]}.json', 'r') as json_datei:
                real_split = json.load(json_datei)
    ...
    if N_SYNTH > 0:
        for split_id in constants.SPLIT_LOOP[0:constants.N_FOLDS]:
            with open(f'../07 train models/synth/{LLM_NAME}/{LLM_SAMPLING}/split_{constants.SPLIT_LOOP[split_id+1]}.json', 'r') as json_datei:
