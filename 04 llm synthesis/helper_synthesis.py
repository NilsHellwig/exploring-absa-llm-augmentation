...
# Valid values for aspect and polarity as per the ABSA task definition
...
def convert_ner_to_xml(ner_dict):
    """
    Converts a NER-style dictionary containing text and tags into an XML-annotated string.
    """
    text = ner_dict['text']
    tags = ner_dict['tags']
    tag_positions = [] 
    ...
    # Sort positions in reverse to insert from end to beginning, maintaining index integrity
...
def get_examples_as_text(examples):
    """
    Formats a list of examples into a human-readable prompt string.
    """
...
def extract_aspect_polarity(xml_string):
    """
    Extracts aspect and polarity attributes from an aspect-term XML tag.
    """
...
def get_explicit_aspects(tags, predicted_text):
    """
    Recursively extracts explicit aspect terms from XML-annotated text and returns them as a list of tags.
    """
    # Use regex to find the first occurring aspect-term
    pattern = r"<aspect-term(?!.*<aspect-term).*?<\/aspect-term>"
    matches = list(re.finditer(pattern, predicted_text))
    if not matches:
        return tags, predicted_text
    ...
    # Calculate position in raw text (without tags)
    tag_text = remove_xml_tags(tag_xml) 
    tag_start = len(remove_xml_tags(predicted_text[:tag_xml_start]))
    tag_end = tag_start + len(tag_text)
    
    # Update text to remove these tags for subsequent processing
    updated_text = predicted_text[:tag_xml_start] + tag_text + predicted_text[tag_xml_end:]
    
    tags.append({
        "text": tag_text, 
        "start": tag_start, 
        "end": tag_end, 
        "tag_with_polarity": f"{aspect}-{polarity}", 
        "tag_with_polarity_and_type": f"{aspect}-{polarity}-explicit", 
        "type": "label-explicit", 
        "label": aspect, 
        "polarity": polarity
    })
    
    return get_explicit_aspects(tags, updated_text)
